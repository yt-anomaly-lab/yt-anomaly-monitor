:root{
  --bg:#0b0d12;
  --text:#e7ebff;
  --muted:#a8b0d8;
  --border:rgba(255,255,255,.08);
  --shadow:0 12px 30px rgba(0,0,0,.35);

  --yellow:#facc15;
  --orange:#fb923c;
  --red:#ef4444;
}

*{box-sizing:border-box}
html,body{height:100%}
body{
  margin:0;
  background:radial-gradient(1200px 800px at 20% 0%, #151b34 0%, var(--bg) 55%);
  color:var(--text);
  font-family: ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, "Noto Sans JP", "Hiragino Kaku Gothic ProN", "Yu Gothic", "Meiryo", sans-serif;
}

/* ===== header ===== */
.topbar{
  display:flex;
  justify-content:space-between;
  align-items:flex-end;
  padding:16px 18px;
  border-bottom:1px solid var(--border);
  background:linear-gradient(180deg, rgba(0,0,0,.25), rgba(0,0,0,0));
}

.title{
  display:flex;
  flex-direction:column;
  gap:4px;
}
.title .h1{font-size:18px; font-weight:700; letter-spacing:.3px}

.power{
  font-size:12px;
  color:rgba(231,235,255,.78);
}

.controls{
  display:flex;
  gap:12px;
  align-items:flex-end;
  flex-wrap:wrap;
}

.ctrl{
  display:flex;
  flex-direction:column;
  gap:6px;
  font-size:12px;
  color:var(--muted);
}
.ctrl select{
  min-width:260px;
  padding:10px 10px;
  border-radius:10px;
  border:1px solid var(--border);
  background:rgba(255,255,255,.04);
  color:var(--text);
  outline:none;
}
.ctrl input{
  min-width:260px;
  padding:10px 10px;
  border-radius:10px;
  border:1px solid var(--border);
  background:rgba(255,255,255,.04);
  color:var(--text);
  outline:none;
}

.manual-row{
  display:flex;
  gap:8px;
  align-items:center;
}
.btn2{
  padding:10px 12px;
  border-radius:10px;
  border:1px solid var(--border);
  background:rgba(255,255,255,.06);
  color:var(--text);
  cursor:pointer;
  font-size:12px;
}
.hint{font-size:11px; color:var(--muted)}
.hidden{display:none !important;}

.toggle{
  display:flex;
  border:1px solid var(--border);
  border-radius:12px;
  overflow:hidden;
  background:rgba(255,255,255,.03);
}
.btn{
  padding:10px 12px;
  border:0;
  background:transparent;
  color:var(--muted);
  cursor:pointer;
  font-size:12px;
}
.btn.active{
  color:var(--text);
  background:rgba(255,255,255,.08);
}
.btn:disabled{
  opacity: 0.45;
  cursor: not-allowed;
}

/* ===== layout ===== */
.layout{
  display:grid;
  grid-template-columns: 1fr 420px;
  grid-template-areas: "main side";
  gap:14px;
  padding:14px;
}

.panel{ grid-area: main; }
.side{  grid-area: side; }

.panel{
  background:linear-gradient(180deg, rgba(255,255,255,.04), rgba(255,255,255,.02));
  border:1px solid var(--border);
  border-radius:18px;
  box-shadow:var(--shadow);
  overflow:hidden;
}

.plot-wrap{
  position: relative;
  height: calc(100vh - 120px);
  min-height: 520px;
}

.plot{ position:absolute; inset:0; }
.plot-pulse{
  position:absolute;
  inset:0;
  width:100%;
  height:100%;
  pointer-events:none;
}

.note{
  padding:10px 14px;
  border-top:1px solid var(--border);
  color:var(--muted);
  font-size:12px;
  background:rgba(0,0,0,.15);
}

.graph-desc{
  margin-top:10px;
  font-size:12px;
  line-height:1.6;
  color:var(--muted);
}

.side{
  display:flex;
  flex-direction:column;
  gap:14px;
}

.card{
  background:linear-gradient(180deg, rgba(255,255,255,.04), rgba(255,255,255,.02));
  border:1px solid var(--border);
  border-radius:18px;
  box-shadow:var(--shadow);
  padding:12px;
}
.card-title{
  font-size:12px;
  color:var(--muted);
  letter-spacing:.3px;
  margin-bottom:10px;
}

.redlist, .chlist{
  display:flex;
  flex-direction:column;
  gap:8px;
  max-height:320px;
  overflow:auto;
  padding-right:6px;
}

/* 読みやすさ */
.item{
  padding:10px;
  border-radius:12px;
  border:1px solid var(--border);
  background:rgba(0,0,0,.18);
  color:var(--text);
}
.item .t{font-size:12px; font-weight:650}
.item .m{
  font-size:11px;
  color:rgba(231,235,255,.85);
  margin-top:4px;
}
.item a{color:inherit; text-decoration:none}
.item a:hover{text-decoration:underline}

.item.active{
  background: rgba(255,255,255,.08);
  border-color: rgba(255,255,255,.18);
}
.item.active .t{ font-weight:800; }

/* ★追加：黄/橙/赤の表示（リスト側も明確に） */
.item.yellow{
  background: rgba(250, 204, 21, .10);
  border-color: rgba(250, 204, 21, .28);
}
.item.orange{
  background: rgba(251, 146, 60, .12);
  border-color: rgba(251, 146, 60, .30);
}
.item.red{
  background: rgba(239, 68, 68, .14);
  border-color: rgba(239, 68, 68, .35);
}

.about{
  font-size:12px;
  color:var(--muted);
  line-height:1.6;
}

/* ===== Mobile順序 ===== */
@media (max-width: 900px){
  .topbar{
    flex-direction:column;
    align-items:flex-start;
    gap:12px;
  }
  .controls{
    width:100%;
    flex-direction:column;
    align-items:flex-start;
  }

  .layout{
    grid-template-columns: 1fr;
    grid-template-areas:
      "main"
      "side";
  }

  .plot-wrap{
    height: 70vh;
    min-height: 420px;
  }

  /* ★要件: グラフの下に並べたい（red→channels→about） */
  .card-red{ order: 1; }
  .card-ch{ order: 2; }
  .card-about{ order: 3; }

  .redlist, .chlist{ max-height: 240px; }
}
