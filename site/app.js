...
(() => {
  ...
  function renderPowerInfo(bundle) {
    ...
    if (!(Number.isFinite(a) && Number.isFinite(bb))) { el.textContent = ""; return; }

    const v1 = Math.exp(a + bb * 1.0);
    if (!Number.isFinite(v1)) { el.textContent = ""; return; }
  ...
  async function waitChannelDataReady(channelId) {
    ...
    for (let i = 0; i < POLL_TRIES_DATA; i++) {
      const ok = await fetchMaybeOk(`${probe}?ts=${Date.now()}`);
      if (ok) return true;
      await sleep(POLL_INTERVAL_MS);
    }
    return false;
  ...
  /* ★デグレ防止：addEventListenerを積み増さない（refreshIndexで何度も呼ばれるため） */
  ...
  async function setChannel(channelId) {
    ...
    renderAnomalyTopList(bundle); // ★red_top依存をやめて「異常値上位」にする
